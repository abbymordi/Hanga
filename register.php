...
<html lang="en">
    ...
    <div id="mainBody">
		...
		<label class="control-label" <?php validateField( "title", $missingFields ) ?> >Title <sup>*</sup></label>
		<div class="controls">
		Mr<input type="radio" name="title" id="Mr" placeholder="" value="Mr"  <?php 
                  setChecked( $member, "title", "Mr" )?> > 
                Mrs<input type="radio" name="title" id="Mrs" placeholder="" value="Mrs"  <?php 
                  setChecked( $member, "title", "Mrs" )?> > 
               Miss<input type="radio" name="title" id="Miss" placeholder="" value="Miss"  <?php 
                  setChecked( $member, "title", "Miss" )?> > 
                                ...
                                <div class="control-group">
                                    <label class="control-label" <?php validateField("lastName", $missingFields) ?>  for="lastName">Last Name<sup>*</sup></label>
                                    <div class="controls">
                                        <input type="text" name="lastName" id="lastName" placeholder="Last Name" value="<?php echo $member->getValueEncoded("lastName") ?>"  > 
                                    </div>
                                </div>
                           ...
                           <div class="control-group">
                                ...
                                <div class="control-group">
                                    ...
                                    <div class="controls">
                                        <input type="text" name="city" id="city" placeholder="Your City" value="<?php echo $member->getValueEncoded("city") ?>"  > 
                                    </div>
                                </div>
                              <div class="control-group">
...
function processForm() {
    $requiredFields = array("title","firstName", "lastName", "email", "password","bio", "city","phone","profile");
    $missingFields = array();
    $errorMessages = array();
 date_default_timezone_set('Africa/Harare');
    ...
    $member = new Member(array(
        ...
        "title" => isset($_POST["title"]) ? preg_replace
                        ("/[^ \-\_a-zA-Z0-9]/", "", $_POST["title"]) : "",
        ...
        "status" => isset($_POST["status"]) ? preg_replace
                        ("/[^ \'\-a-zA-Z0-9]/", "", $_POST["status"]) : "",
...
// }else{
 ...
 if (move_uploaded_file($_FILES['profile']['tmp_name'],$target)){
    ...
    if ((empty($_POST['lastName'])) 
               ...
               {
             $errorMessages[] = '<p class="error"> Last Name is not filled </p>';
           }   
    
  if ((empty($_POST['title']))
                     ) 
               {
               $errorMessages[] = '<p class="error"> The title is not filled </p>';
           }  
    if ((empty($_POST['firstName']))
                     ) 
               {
               $errorMessages[] = '<p class="error"> The First Name is not filled </p>';
           }   
     if ((empty($_POST['email']))
      ...
      if ((empty($_POST['phone']))
        ...
        if ((empty($_POST['city']))
               ...
               {
               $errorMessages[] = '<p class="error"> Your City is required  </p>';
           }        
         ...
         if ((empty($_POST['province']))
               ...
               {
               $errorMessages[] = '<p class="error"> Your province is required  </p>';
           }      
    ...
    if ( Member::getByPhone( $member->getValue( "phone" ) ) ) {
     $errorMessages[] = '<p class="error"> A member with that phone number
already exists.  </p>';
     }
...
contact the Hanga admin for more info. </p>';
    }
